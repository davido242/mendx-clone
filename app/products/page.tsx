...
export default function page() {
  ...
  const imgRefBack = useRef<HTMLInputElement | null>(null);
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    body.set("frontImage", imgRef.current.files[0] ?? "");
    body.set("backImage", imgRefBack.current.files[0] ?? "");

    console.log("Image thing:", imgRefBack.current?.files[0]);
    // return;

    fetch("http://localhost:5001/product/add-product", {
      method: "POST",
      headers: {
      ...
      <div className="container mx-auto px-8">
        <div className="bg-brown-bg mt-32 p-4 rounded max-w-[500px] mx-auto">
          ...
          <form onSubmit={handleSubmit} encType="multipart/form-data" className="flex flex-col gap-4">
            ...
            <input type="file" ref={imgRef} name="frontImage" className="cursor-pointer bg-[#efefef] p-2 form-input" />
            <label htmlFor="backImage">Back Image</label>
            <input type="file" ref={imgRefBack} name="backImage" className="cursor-pointer bg-[#efefef] p-2 form-input" />
